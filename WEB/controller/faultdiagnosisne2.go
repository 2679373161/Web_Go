...
type Ifaultdiagnosisne2Controller interface{
	RestController
...
type faultdiagnosisne2Controller struct {
	DB *gorm.DB
}
func (t faultdiagnosisne2Controller) Create(ctx *gin.Context) {
...
func (t faultdiagnosisne2Controller) Update(ctx *gin.Context) {
	var requestTableStoreDate vo.CreateDatasaveRequest
	//数据验证
	if err:=ctx.ShouldBind(&requestTableStoreDate);err!=nil{
		log.Println(err.Error())
...
func (t faultdiagnosisne2Controller) Show(ctx *gin.Context) {
	tableStoreDateId:=ctx.Params.ByName("id") //从上下文中解析
...
func (t faultdiagnosisne2Controller) Delete(ctx *gin.Context) {
	tableStoreDateId:=ctx.Params.ByName("id") //从上下文中解析
...
func (t faultdiagnosisne2Controller) Search(ctx *gin.Context) {
	fmt.Println("历史详情")
    ...
    var downloadplace [] model.Heatfaultne2
...
if downloadflag=="0"{
	if citycode!="0"&&len(devtype)!=0&&len(equipment)!=0{
		...
		common.IndexDB.Table(table).Where("province_code= ? and city_code=? and dev_type= ? and dev_id= ? and start_time >= ? and start_time <= ?  ", provincecode,citycode,devtype, equipment, starttime, endtime).Count(&count)
		common.IndexDB.Table(table).Where("province_code=? and city_code=?  and dev_type=? and dev_id=? and start_time >= ? and start_time <=?  ", provincecode, citycode,devtype, equipment, starttime, endtime).Offset(perPage * (currentPage - 1)).Limit(perPage).Find(&downloadplace)
	}

	if citycode == "0" {
		common.IndexDB.Table(table).Where("province_code= ?  and dev_type= ? and dev_id= ? and start_time >= ? and start_time <= ?  ", provincecode, devtype, equipment, starttime, endtime).Count(&count)
		common.IndexDB.Table(table).Where("province_code=?  and dev_type=? and dev_id=? and start_time >= ? and start_time <=?  ", provincecode, devtype, equipment, starttime, endtime).Offset(perPage * (currentPage - 1)).Limit(perPage).Find(&downloadplace)
	}
	if len(devtype) == 0 {
		common.IndexDB.Table(table).Where("province_code= ?  and city_code= ? and dev_id= ? and start_time >= ? and start_time <= ?  ", provincecode, citycode, equipment, starttime, endtime).Count(&count)
		common.IndexDB.Table(table).Where("province_code=?  and city_code=? and dev_id=? and start_time >= ? and start_time <=?  ", provincecode, citycode, equipment, starttime, endtime).Offset(perPage * (currentPage - 1)).Limit(perPage).Find(&downloadplace)
	}
	if len(equipment) == 0 {
		common.IndexDB.Table(table).Where("province_code= ?  and city_code= ? and dev_type= ? and start_time >= ? and start_time <= ?  ", provincecode, citycode, devtype, starttime, endtime).Count(&count)
		common.IndexDB.Table(table).Where("province_code=?  and city_code=? and dev_type=? and start_time >= ? and start_time <=?  ", provincecode, citycode, devtype, starttime, endtime).Offset(perPage * (currentPage - 1)).Limit(perPage).Find(&downloadplace)
	}
	if provincecode == "0" {
		common.IndexDB.Table(table).Where(" dev_type= ? and dev_id= ? and start_time >= ? and start_time <= ?  ", devtype, equipment, starttime, endtime).Count(&count)
		common.IndexDB.Table(table).Where("dev_type=? and dev_id=? and start_time >= ? and start_time <=?  ", devtype, equipment, starttime, endtime).Offset(perPage * (currentPage - 1)).Limit(perPage).Find(&downloadplace)
	}
	if citycode == "0" && (len(devtype) == 0) {
		common.IndexDB.Table(table).Where("province_code= ?  and dev_id= ? and start_time >= ? and start_time <= ?  ", provincecode, equipment, starttime, endtime).Count(&count)
		common.IndexDB.Table(table).Where("province_code=?   and dev_id=? and start_time >= ? and start_time <=?  ", provincecode, equipment, starttime, endtime).Offset(perPage * (currentPage - 1)).Limit(perPage).Find(&downloadplace)
	}
	if citycode == "0" && len(equipment) == 0 {
		common.IndexDB.Table(table).Where("province_code= ?  and dev_type= ?  and start_time >= ? and start_time <= ?  ", provincecode, devtype, starttime, endtime).Count(&count)
		common.IndexDB.Table(table).Where("province_code=? and  dev_type=?  and start_time >= ? and start_time <=?  ", provincecode, devtype, starttime, endtime).Offset(perPage * (currentPage - 1)).Limit(perPage).Find(&downloadplace)
	}
	if provincecode == "0" && len(devtype) == 0 {
		common.IndexDB.Table(table).Where("  dev_id= ? and start_time >= ? and start_time <= ?  ", equipment, starttime, endtime).Count(&count)
		common.IndexDB.Table(table).Where(" dev_id=? and start_time >= ? and start_time <=?  ", equipment, starttime, endtime).Offset(perPage * (currentPage - 1)).Limit(perPage).Find(&downloadplace)
	}
	if provincecode == "0" && len(equipment) == 0 {
		common.IndexDB.Table(table).Where("dev_type= ? and start_time >= ? and start_time <= ?  ", devtype, starttime, endtime).Count(&count)
		common.IndexDB.Table(table).Where("dev_type=? and start_time >= ? and start_time <=?  ", devtype, starttime, endtime).Offset(perPage * (currentPage - 1)).Limit(perPage).Find(&downloadplace)
	}
	if len(devtype) == 0 && len(equipment) == 0 {
		common.IndexDB.Table(table).Where("province_code= ? and city_code= ?  and  start_time >= ? and start_time <= ?  ", provincecode, citycode, starttime, endtime).Count(&count)
		common.IndexDB.Table(table).Where("province_code=?  and city_code= ? and start_time >= ? and start_time <=?  ", provincecode, citycode, starttime, endtime).Offset(perPage * (currentPage - 1)).Limit(perPage).Find(&downloadplace)
	}
	if citycode == "0" && len(devtype) == 0 && len(equipment) == 0 {
		common.IndexDB.Table(table).Where("province_code= ?   and start_time >= ? and start_time <= ?  ", provincecode, starttime, endtime).Count(&count)
		common.IndexDB.Table(table).Where("province_code=?  and start_time >= ? and start_time <=?  ", provincecode, starttime, endtime).Offset(perPage * (currentPage - 1)).Limit(perPage).Find(&downloadplace)
	}
	if provincecode == "0" && len(devtype) == 0 && len(equipment) == 0 {
		common.IndexDB.Table(table).Where("start_time >= ? and start_time <= ?  ", starttime, endtime).Count(&count)
		common.IndexDB.Table(table).Where("start_time >= ? and start_time <=?  ", starttime, endtime).Offset(perPage * (currentPage - 1)).Limit(perPage).Find(&downloadplace)
	...
	if citycode!="0"&&len(devtype)!=0&&len(equipment)!=0{
		...
		common.IndexDB.Table(table).Where("province_code=? and city_code=?  and dev_type=? and dev_id=? and start_time >= ? and start_time <=?  ", provincecode, citycode,devtype, equipment, starttime, endtime).Find(&downloadplace)
	}

	if citycode == "0" {
		common.IndexDB.Table(table).Where("province_code=?  and dev_type=? and dev_id=? and start_time >= ? and start_time <=?  ", provincecode, devtype, equipment, starttime, endtime).Find(&downloadplace)
	}
	if len(devtype) == 0 {
		common.IndexDB.Table(table).Where("province_code=?  and city_code=? and dev_id=? and start_time >= ? and start_time <=?  ", provincecode, citycode, equipment, starttime, endtime).Find(&downloadplace)
	}
	if len(equipment) == 0 {
		common.IndexDB.Table(table).Where("province_code=?  and city_code=? and dev_type=? and start_time >= ? and start_time <=?  ", provincecode, citycode, devtype, starttime, endtime).Find(&downloadplace)
	}
	if provincecode == "0" {
		common.IndexDB.Table(table).Where("dev_type=? and dev_id=? and start_time >= ? and start_time <=?  ", devtype, equipment, starttime, endtime).Find(&downloadplace)
	}
	if citycode == "0" && (len(devtype) == 0) {
		common.IndexDB.Table(table).Where("province_code=?   and dev_id=? and start_time >= ? and start_time <=?  ", provincecode, equipment, starttime, endtime).Find(&downloadplace)
	}
	if citycode == "0" && len(equipment) == 0 {
		common.IndexDB.Table(table).Where("province_code=? and  dev_type=?  and start_time >= ? and start_time <=?  ", provincecode, devtype, starttime, endtime).Find(&downloadplace)
	}
	if provincecode == "0" && len(devtype) == 0 {
		common.IndexDB.Table(table).Where(" dev_id=? and start_time >= ? and start_time <=?  ", equipment, starttime, endtime).Find(&downloadplace)
	}
	if provincecode == "0" && len(equipment) == 0 {
		common.IndexDB.Table(table).Where("dev_type=? and start_time >= ? and start_time <=?  ", devtype, starttime, endtime).Find(&downloadplace)
	}
	if len(devtype) == 0 && len(equipment) == 0 {
		common.IndexDB.Table(table).Where("province_code=?  and city_code= ? and start_time >= ? and start_time <=?  ", provincecode, citycode, starttime, endtime).Find(&downloadplace)
	}
	if citycode == "0" && len(devtype) == 0 && len(equipment) == 0 {
		common.IndexDB.Table(table).Where("province_code=?  and start_time >= ? and start_time <=?  ", provincecode, starttime, endtime).Find(&downloadplace)
	}
	if provincecode == "0" && len(devtype) == 0 && len(equipment) == 0 {
		common.IndexDB.Table(table).Where("start_time >= ? and start_time <=?  ", starttime, endtime).Find(&downloadplace)
...
func (t faultdiagnosisne2Controller) Summaries(ctx *gin.Context) {
	fmt.Println("故障诊断2")
	//downloadflag:=ctx.DefaultQuery("downloadflag", "0")
	flag := ctx.DefaultQuery("flag", "1")
	provincecode := ctx.DefaultQuery("province_code", "0")
	...
	//var tableplace1  []model.Heatfaultne2
	var count int
	...
	if provincecode == "0" && len(devtype) == 0 && len(equipment) == 0 {
		...
		fmt.Println(table)
		//common.IndexDB.Raw("select * from e2_recent where time_date=2022-04-04").Find(downloadplace)
	//	timedate="\""+timedate+"\""
...
func (t faultdiagnosisne2Controller) Getequipment(ctx *gin.Context) {
	...
	if flag == "3" {
		...
		//common.IndexDB.Table("Heatfaultne2").Where("province_code = ? and dev_type = ? ", province_code, dev_type).Find(&tableStoreDates4)
		...
		//common.IndexDB.Table("Heatfaultne2").Where("city_code = ? and dev_type = ? ", city_code, dev_type).Find(&tableStoreDates4)
		fmt.Println(tableStoreDates4)
		for _, tableDate := range tableStoreDates4 {
			var tableStoreDates3 Equipment1
			tableStoreDates3.Value = tableDate.Dev_id
		...
		//common.IndexDB.Table("Heatfaultne2").Where("dev_type = ? ", dev_type).Find(&tableStoreDates4)
		fmt.Println(tableStoreDates4)
		for _, tableDate := range tableStoreDates4 {
			var tableStoreDates3 Equipment1
			tableStoreDates3.Value = tableDate.Dev_id
...
func (t faultdiagnosisne2Controller) Ceshi(ctx *gin.Context) {
	type Types1 struct {
...
func (t faultdiagnosisne2Controller) Gettype(ctx *gin.Context) {
	flag := ctx.DefaultQuery("flag", "0")
	province_code := ctx.DefaultQuery("province_code", "0")
	city_code := ctx.DefaultQuery("city_code", "0")
...
func (t faultdiagnosisne2Controller) Getday(ctx *gin.Context) {
	fmt.Println("666")
	var day_infor [] model.Heatfaultne2
	flag := ctx.DefaultQuery("flag", "0")
	equipment := ctx.DefaultQuery("equipment", "")
	...
	func Newfaultdiagnosisne2Controller ()Ifaultdiagnosisne2Controller{
	db:=common.GetDB()
	db.AutoMigrate(model.TableDate{})
	return faultdiagnosisne2Controller{DB:db}
}